...
export default function InvoiceModal({ deliveries, customer, settings, onClose, invoiceNumber, monthlyTotalPaid, monthlyBalanceDue }) {
  ...
  function getQrScript(jazzcash, easypaisa) {
    return `
...
(function () {
    ...
    window.onload = function() {
      ${jazzcash ? `var el1 = document.getElementById('qr-jazzcash'); if(el1) new QRCode(el1, { text: 'JazzCash: ${jazzcash}', width: 90, height: 90, correctLevel: QRCode.CorrectLevel.M });` : ''}
      ${easypaisa ? `var el2 = document.getElementById('qr-easypaisa'); if(el2) new QRCode(el2, { text: 'EasyPaisa: ${easypaisa}', width: 90, height: 90, correctLevel: QRCode.CorrectLevel.M });` : ''}
      setTimeout(function() { window.print(); window.onafterprint = function() { window.close(); }; }, 1500);
    };
    `
  }
  ...
  function printA4() {
    ...
    const qrScript = getQrScript(settings.jazzcash_number, settings.easypaisa_number)
      ...
      <div style={{ background: '#f5f6fa', borderRadius: '16px', padding: '20px', maxWidth: '860px', width: '100%', maxHeight: '95vh', overflow: 'auto' }}>
        ...
        <div ref={printRef} style={{ background: 'white', borderRadius: '12px', padding: '32px', marginBottom: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', gap: '16px' }}>
            ...
            {/* QR Codes — left side (placeholders, filled by inline script in print window) */}
            {(settings.jazzcash_number || settings.easypaisa_number) && (
              <div style={{ display: 'flex', gap: '14px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                {settings.jazzcash_number && (
                  <div style={{ textAlign: 'center' }}>
                    <div id="qr-jazzcash" style={{ width: '90px', height: '90px', border: '2px solid #e65100', borderRadius: '6px', overflow: 'hidden', marginBottom: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff8f0', fontSize: '9px', color: '#e65100' }}>QR</div>
                ...
                {settings.easypaisa_number && (
                  <div style={{ textAlign: 'center' }}>
                    <div id="qr-easypaisa" style={{ width: '90px', height: '90px', border: '2px solid #1a7a4a', borderRadius: '6px', overflow: 'hidden', marginBottom: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f0fff4', fontSize: '9px', color: '#1a7a4a' }}>QR</div>
