...
export default function RiderReceivables({ rider, tenantId, onSelectCustomer, isOnline, dbReady }) {
  ...
  useEffect(() => { if (tenantId) fetchReceivables() }, [isOnline, tenantId])
          ...
          {filtered.map(c => {
            ...
            return (
              <div key={c.id}
                ...
                style={{ background: 'white', borderRadius: '12px', padding: '14px 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', cursor: 'pointer', border: c.hasActiveOrder ? '2px solid #0f4c81' : '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ flex: 1 }}>
