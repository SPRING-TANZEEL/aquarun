...
export default function RiderCashSummary({ rider, tenantId, lang = 'en' }) {
  const [summary, setSummary] = useState(null)
  const [totalUncleared, setTotalUncleared] = useState(null)
  ...
  async function fetchSummary() {
    ...
    const { data: lastTransfer } = await supabase.from('cash_transfers')
      .select('confirmed_at, amount').eq('tenant_id', tenantId)
      .eq('from_rider_id', rider.id).eq('to_office', true).eq('status', 'confirmed')
    ...
    setSummary({
      ...
      totalCashIn, totalExpenses, cashInHand,
    ...
    const { data: allDeliveries } = await supabase.from('deliveries')
      .select('amount_received, payment_method').eq('rider_id', rider.id).eq('tenant_id', tenantId).eq('is_voided', false)
  ...
  function toggleSection(key) { setExpandedSection(expandedSection === key ? null : key) }
  ...
  const dateLabel = mode === 'today'
    ...
    : lastTransferDate
      ? `${t('Since last transfer', 'آخری منتقلی کے بعد سے')} (${new Date(lastTransferDate).toLocaleDateString('en-PK', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })})`
      : t('All time — no transfer recorded yet', 'تمام وقت — ابھی تک کوئی منتقلی نہیں')

  return (
    <div>
      <h2 style={{ fontSize: '18px', fontWeight: '700', color: '#333', margin: '0 0 12px' }}>💰 {t('Cash Summary', 'کیش خلاصہ')}</h2>

      {/* ── TWO CARD SUMMARY (like admin) ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '16px' }}>
        {/* Today card */}
        <div style={{ background: summary.cashInHand > 0 ? 'linear-gradient(135deg, #0f4c81, #1565c0)' : 'linear-gradient(135deg, #1a7a4a, #2e7d32)', borderRadius: '14px', padding: '16px', color: 'white', textAlign: 'center' }}>
          <p style={{ fontSize: '11px', opacity: 0.8, margin: '0 0 4px', textTransform: 'uppercase' }}>{t('Today', 'آج')}</p>
          <p style={{ fontSize: '26px', fontWeight: '700', margin: '0 0 4px' }}>Rs. {Math.max(0, summary.cashInHand).toLocaleString()}</p>
          <p style={{ fontSize: '10px', opacity: 0.7, margin: 0 }}>
            {t('Sales', 'فروخت')} {summary.cashFromSales.toLocaleString()} + {t('Col', 'وصولی')} {summary.cashFromPayments.toLocaleString()} − {t('Exp', 'خرچ')} {summary.totalExpenses.toLocaleString()}
          </p>
        </div>
        {/* Total uncleared card */}
        <div style={{ background: totalUncleared && totalUncleared.uncleared > 0 ? 'linear-gradient(135deg, #c62828, #e65100)' : 'linear-gradient(135deg, #1a7a4a, #0f4c81)', borderRadius: '14px', padding: '16px', color: 'white', textAlign: 'center' }}>
          <p style={{ fontSize: '11px', opacity: 0.8, margin: '0 0 4px', textTransform: 'uppercase' }}>{t('Total Uncleared', 'کل غیر منتقل')}</p>
          <p style={{ fontSize: '26px', fontWeight: '700', margin: '0 0 4px' }}>Rs. {totalUncleared ? totalUncleared.uncleared.toLocaleString() : '...'}</p>
          <p style={{ fontSize: '10px', opacity: 0.7, margin: 0 }}>
            {t('Sales+Col−Exp−Transferred', 'فروخت+وصولی−خرچ−منتقل')}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
        ...
        <button onClick={() => setMode('since_transfer')}
          ...
          📊 {t('Since Last Transfer', 'آخری منتقلی کے بعد')}
        </button>
      </div>
      ...
      <div style={{ background: '#f0f7ff', borderRadius: '10px', padding: '10px 14px', marginBottom: '12px' }}>
        ...
        {mode === 'since_transfer' && !lastTransferDate && (
          <p style={{ fontSize: '11px', color: '#e65100', margin: '4px 0 0' }}>⚠️ {t('No transfer recorded yet — showing all cash since start', 'ابھی تک کوئی منتقلی نہیں — شروع سے تمام کیش')}</p>
        )}
      </div>

      {/* CASH IN */}
      <div style={{ background: 'white', borderRadius: '12px', padding: '16px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        <p style={{ fontSize: '13px', fontWeight: '700', color: '#1a7a4a', marginBottom: '4px' }}>📥 {t('CASH IN', 'کیش آمد')}</p>
        <p style={{ fontSize: '11px', color: '#888', margin: '0 0 12px' }}>{t('Tap any row to see details', 'تفصیل کے لیے کسی قطار پر ٹیپ کریں')}</p>

        <SectionHeader sectionKey="cashDeliveries" title={t('Cash from Deliveries', 'ڈیلیوری سے کیش')} amount={summary.cashFromSales} color="#1a7a4a" count={summary.cashDeliveries.length} />
        {expandedSection === 'cashDeliveries' && (
          <div style={{ paddingTop: '10px', paddingBottom: '4px' }}>
            {summary.cashDeliveries.length === 0
              ? <p style={{ fontSize: '12px', color: '#888', textAlign: 'center', padding: '10px 0' }}>{t('No cash deliveries', 'کوئی نقد ڈیلیوری نہیں')}</p>
              : summary.cashDeliveries.map(d => <DeliveryRow key={d.id} d={d} />)}
          </div>
        )}

        <SectionHeader sectionKey="cashPayments" title={t('Cash from Balance Collections', 'بقایا وصولی سے کیش')} amount={summary.cashFromPayments} color="#1a7a4a" count={summary.cashPayments.length} />
        {expandedSection === 'cashPayments' && (
          <div style={{ paddingTop: '10px', paddingBottom: '4px' }}>
            {summary.cashPayments.length === 0
              ? <p style={{ fontSize: '12px', color: '#888', textAlign: 'center', padding: '10px 0' }}>{t('No cash collections', 'کوئی نقد وصولی نہیں')}</p>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0 0', marginTop: '4px', borderTop: '2px solid #e8f5e9' }}>
          <span style={{ fontSize: '14px', fontWeight: '700', color: '#333' }}>{t('Total Cash In', 'کل کیش آمد')}</span>
      ...
      {/* CASH OUT */}
      <div style={{ background: 'white', borderRadius: '12px', padding: '16px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        <p style={{ fontSize: '13px', fontWeight: '700', color: '#f44336', marginBottom: '12px' }}>📤 {t('CASH OUT', 'کیش خروج')}</p>

        <SectionHeader sectionKey="expenses" title={t('Field Expenses', 'فیلڈ خرچے')} amount={summary.totalExpenses} color="#e65100" count={summary.expenses.length} />
        {expandedSection === 'expenses' && (
          <div style={{ paddingTop: '10px', paddingBottom: '4px' }}>
            {summary.expenses.length === 0
              ? <p style={{ fontSize: '12px', color: '#888', textAlign: 'center', padding: '10px 0' }}>{t('No expenses', 'کوئی خرچہ نہیں')}</p>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0 0', marginTop: '4px', borderTop: '2px solid #ffebee' }}>
          <span style={{ fontSize: '14px', fontWeight: '700', color: '#333' }}>{t('Total Expenses', 'کل خرچے')}</span>
          <span style={{ fontSize: '18px', fontWeight: '700', color: '#f44336' }}>Rs. {summary.totalExpenses.toLocaleString()}</span>
        </div>
      </div>

      {/* Transfers (informational) */}
      {(summary.totalSent > 0 || summary.totalReceived > 0) && (
        <div style={{ background: 'white', borderRadius: '12px', padding: '16px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: '1px solid #e3f0ff' }}>
          ...
          {summary.totalSent > 0 && (
            <>
              ...
              {expandedSection === 'sentTransfers' && (
                <div style={{ paddingTop: '10px', paddingBottom: '4px' }}>
                  {summary.sentTransfers.map(tr => (
                    <div key={tr.id} style={{ padding: '10px 12px', background: '#f0f7ff', borderRadius: '8px', marginBottom: '6px', borderLeft: '3px solid #0f4c81', display: 'flex', justifyContent: 'space-between' }}>
                      <div>
                        <p style={{ fontSize: '13px', fontWeight: '600', margin: '0 0 2px' }}>{tr.to_office ? `🏢 ${t('To Office', 'دفتر کو')}` : `⭐ ${t('To Main Rider', 'مین رائیڈر کو')}`}</p>
                        <p style={{ fontSize: '11px', color: '#888', margin: 0 }}>{new Date(tr.confirmed_at).toLocaleTimeString('en-PK', { hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
                      <p style={{ fontSize: '14px', fontWeight: '700', color: '#0f4c81', margin: 0 }}>Rs. {Number(tr.amount).toLocaleString()}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          {summary.totalReceived > 0 && (
            <>
              <SectionHeader sectionKey="receivedTransfers" title={t('Received from Riders', 'رائیڈرز سے موصول')} amount={summary.totalReceived} color="#0f4c81" count={summary.receivedTransfers.length} />
              {expandedSection === 'receivedTransfers' && (
                <div style={{ paddingTop: '10px', paddingBottom: '4px' }}>
                  {summary.receivedTransfers.map(tr => (
                    <div key={tr.id} style={{ padding: '10px 12px', background: '#f0f7ff', borderRadius: '8px', marginBottom: '6px', borderLeft: '3px solid #0f4c81', display: 'flex', justifyContent: 'space-between' }}>
                      <p style={{ fontSize: '13px', color: '#555', margin: 0 }}>{t('Transfer received', 'منتقلی موصول')}</p>
                      <p style={{ fontSize: '14px', fontWeight: '700', color: '#0f4c81', margin: 0 }}>Rs. {Number(tr.amount).toLocaleString()}</p>
                    </div>
                  ))}
                </div>
      ...
      {/* NON-CASH */}
      <div style={{ background: 'white', borderRadius: '12px', padding: '16px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        <p style={{ fontSize: '13px', fontWeight: '700', color: '#9c27b0', marginBottom: '12px' }}>📱 {t('NON-CASH (Informational)', 'غیر نقد (معلوماتی)')}</p>

        <SectionHeader sectionKey="jazz" title={t('JazzCash Sales', 'جیز کیش فروخت')} amount={summary.jazzFromSales + summary.jazzFromSalesPending} color="#9c27b0" count={summary.jazzDeliveries.length} />
        {expandedSection === 'jazz' && (
          <div style={{ paddingTop: '10px', paddingBottom: '4px' }}>
            {summary.jazzDeliveries.length === 0
              ? <p style={{ fontSize: '12px', color: '#888', textAlign: 'center', padding: '10px 0' }}>{t('No JazzCash entries', 'کوئی جیز کیش اندراج نہیں')}</p>
              : summary.jazzDeliveries.map(d => <DeliveryRow key={d.id} d={d} />)}
          </div>
        )}

        <SectionHeader sectionKey="credit" title={t('Credit Sales (ادھار)', 'ادھار فروخت')} amount={summary.creditSales} color="#f44336" count={summary.creditDeliveries.length} />
        {expandedSection === 'credit' && (
          <div style={{ paddingTop: '10px', paddingBottom: '4px' }}>
            {summary.creditDeliveries.length === 0
              ? <p style={{ fontSize: '12px', color: '#888', textAlign: 'center', padding: '10px 0' }}>{t('No credit sales', 'کوئی ادھار فروخت نہیں')}</p>
      ...
      <div style={{ background: 'white', borderRadius: '12px', padding: '16px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: '2px solid #e3f0ff' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0 4px', borderTop: '2px solid #0f4c81', marginTop: '4px' }}>
          <span style={{ fontSize: '15px', fontWeight: '700', color: '#0f4c81' }}>{t('Cash in Hand', 'ہاتھ میں کیش')}</span>
        ...
        <p style={{ fontSize: '11px', color: '#888', margin: '6px 0 0', textAlign: 'center' }}>
          {t('Count your physical cash and match with above', 'اپنا جسمانی کیش گنیں اور اوپر سے ملائیں')}
        </p>
      </div>
