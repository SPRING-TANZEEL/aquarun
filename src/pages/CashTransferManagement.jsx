...
export default function CashTransferManagement({ tenantId, onUpdate }) {
  ...
  async function fetchData() {
    ...
    for (const r of riders || []) {
      ...
      // ── TODAY ──
      const { data: todayDeliveries } = await supabase.from('deliveries')
        .select('amount_received, payment_method')
        .eq('rider_id', r.id).eq('tenant_id', tenantId)
        .eq('is_voided', false)
        .gte('delivered_at', todayFrom).lte('delivered_at', todayTo)

      const { data: todayPayments } = await supabase.from('payments')
      ...
      const todayCollections = todayPayments?.reduce((s, p) => s + Number(p.amount), 0) || 0
    ...
    const { data: pendingData } = await supabase.from('cash_transfers')
      ...
      .eq('tenant_id', tenantId).eq('to_office', true).eq('status', 'pending')
    ...
    if (filter === 'pending') { setPendingTransfers(pendingData || []); setConfirmedTransfers([]) }
    else if (filter === 'confirmed') { setPendingTransfers([]); setConfirmedTransfers(confirmedData || []) }
    else { setPendingTransfers(pendingData || []); setConfirmedTransfers(confirmedData || []) }
  ...
  async function confirmTransfer(transfer) {
    ...
    const { data: confirmed, error } = await supabase.from('cash_transfers')
  ...
  async function rejectTransfer(transfer) {
    setConfirming(transfer.id)
    await supabase.from('cash_transfers').update({ status: 'rejected' })
    ...
    <div>
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '24px', width: '100%', maxWidth: '420px', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: '700', color: '#333', margin: 0 }}>
                {selectedRider.is_main_rider ? '⭐' : '🚴'} {selectedRider.full_name}
              </h3>
              <button onClick={() => setSelectedRider(null)} style={{ background: 'none', border: 'none', fontSize: '22px', cursor: 'pointer', color: '#888' }}>✕</button>
            </div>

            {/* Today breakdown */}
            <div style={{ background: '#f0f7ff', borderRadius: '10px', padding: '14px', marginBottom: '14px' }}>
              <p style={{ fontSize: '12px', fontWeight: '700', color: '#0f4c81', margin: '0 0 10px', textTransform: 'uppercase' }}>Today</p>
              {[
                { label: '💵 Cash Sales', value: selectedRider.todayCashSales, color: '#1a7a4a' },
                { label: '💰 Collections', value: selectedRider.todayCollections, color: '#1a7a4a' },
                { label: '🧾 Expenses', value: -selectedRider.todayExp, color: '#f44336' },
              ].map(row => (
                <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid #e3f0ff' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0 0' }}>
                <span style={{ fontSize: '14px', fontWeight: '700', color: '#0f4c81' }}>Today's Cash in Hand</span>
                <span style={{ fontSize: '16px', fontWeight: '700', color: selectedRider.todayBalance >= 0 ? '#0f4c81' : '#f44336' }}>
                  Rs. {selectedRider.todayBalance.toLocaleString()}
                </span>
              </div>
            </div>

            {/* All time uncleared breakdown */}
            <div style={{ background: selectedRider.totalUncleared > 0 ? '#fff8e1' : '#f0fff4', borderRadius: '10px', padding: '14px' }}>
              <p style={{ fontSize: '12px', fontWeight: '700', color: selectedRider.totalUncleared > 0 ? '#e65100' : '#1a7a4a', margin: '0 0 10px', textTransform: 'uppercase' }}>Total Uncleared Cash</p>
              {[
                { label: '💵 Total Cash Sales', value: selectedRider.totalCashSales, color: '#1a7a4a' },
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0 0', borderTop: '2px solid rgba(0,0,0,0.1)', marginTop: '4px' }}>
                <span style={{ fontSize: '14px', fontWeight: '700', color: '#333' }}>Net Cash Still Holding</span>
                <span style={{ fontSize: '18px', fontWeight: '700', color: selectedRider.totalUncleared > 0 ? '#e65100' : '#1a7a4a' }}>
                  Rs. {selectedRider.totalUncleared.toLocaleString()}
                </span>
              </div>
            </div>
          </div>
      ...
      <div style={{ background: 'white', borderRadius: '12px', padding: '16px', marginBottom: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <p style={{ fontSize: '13px', fontWeight: '700', color: '#555', margin: 0 }}>Rider Cash Summary</p>
          <span style={{ fontSize: '11px', color: '#888', background: '#f0f0f0', padding: '3px 8px', borderRadius: '6px' }}>Tap card for details</span>
        </div>
        {riderBalances.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '10px' }}>
            {riderBalances.map(r => (
              <div key={r.id} onClick={() => setSelectedRider(r)}
                style={{
                  padding: '14px', borderRadius: '10px', cursor: 'pointer',
                  background: r.totalUncleared > 0 ? '#fff8e1' : '#f0fff4',
                  border: '1px solid ' + (r.totalUncleared > 0 ? '#ffe082' : '#c8e6c9'),
                  transition: 'transform 0.1s', boxShadow: '0 2px 6px rgba(0,0,0,0.06)'
                }}
                onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.02)'}
                onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}
              >
                <p style={{ fontSize: '13px', fontWeight: '700', color: '#333', margin: '0 0 8px' }}>
                  {r.is_main_rider ? '⭐ ' : '🚴 '}{r.full_name}
                </p>

                {/* Today */}
                <div style={{ marginBottom: '8px', paddingBottom: '8px', borderBottom: '1px dashed rgba(0,0,0,0.1)' }}>
                  <p style={{ fontSize: '10px', color: '#888', margin: '0 0 2px', textTransform: 'uppercase' }}>Today</p>
                  <p style={{ fontSize: '20px', fontWeight: '700', margin: 0, color: r.todayBalance > 0 ? '#e65100' : '#1a7a4a' }}>
                    Rs. {r.todayBalance.toLocaleString()}
                  </p>
                </div>

                {/* Total Uncleared */}
                <div>
                  <p style={{ fontSize: '10px', color: '#888', margin: '0 0 2px', textTransform: 'uppercase' }}>Total Uncleared</p>
        ...
        <div style={{ marginTop: '12px', padding: '10px 14px', background: '#0f4c81', borderRadius: '8px', display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ fontSize: '13px', color: 'white', fontWeight: '600' }}>Total Uncleared Cash (All Riders)</span>
            ...
            <div style={{ marginBottom: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                ...
                {pendingTransfers.length > 0 && (
                  <span style={{ background: '#e65100', color: 'white', fontSize: '11px', padding: '2px 8px', borderRadius: '10px', fontWeight: '700' }}>{pendingTransfers.length}</span>
                )}
              </div>
                ...
                {pendingTransfers.length === 0 ? (
                  ...
                  <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', borderBottom: '1px solid #f0f0f0', background: '#fffbf5' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <p style={{ fontSize: '20px', fontWeight: '700', color: '#0f4c81', margin: 0 }}>Rs. {Number(t.amount).toLocaleString()}</p>
                      <div style={{ display: 'flex', gap: '8px' }}>
                ...
                {confirmedTransfers.map(t => (
                  <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', borderBottom: '1px solid #f0f0f0', background: '#fafffe' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <p style={{ fontSize: '20px', fontWeight: '700', color: '#1a7a4a', margin: 0 }}>Rs. {Number(t.amount).toLocaleString()}</p>
