...
export default function SalaryManagement({ adminUser }) {
  ...
  async function fetchData() {
    ...
    const { data: ridersData } = await supabase
      .from('riders').select('*').eq('is_active', true).order('created_at')
    ...
    // Calculate summaries for each rider
          ...
          {riderSummaries.map(r => (
            <div key={r.id} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                ...
                <button onClick={() => { setPayingRider(r); setPayAmount(String(r.remaining > 0 ? r.remaining : 0)) }}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                <div style={{ textAlign: 'center', padding: '10px', background: '#f0f7ff', borderRadius: '8px' }}>
                  <p style={{ fontSize: '11px', color: '#888', margin: '0 0 4px' }}>
                    {r.salary_type === 'commission' ? 'Commission Earned' : 'Monthly Salary'}
                  </p>
                ...
                <div style={{ textAlign: 'center', padding: '10px', background: r.remaining >= 0 ? '#e8f5e9' : '#ffebee', borderRadius: '8px' }}>
                  ...
                  <p style={{ fontSize: '18px', fontWeight: '700', color: r.remaining >= 0 ? '#1a7a4a' : '#c62828', margin: 0 }}>
                    Rs. {r.remaining.toLocaleString()}
                  </p>
                </div>
              </div>
              ...
              {r.remaining < 0 && (
                <div style={{ background: '#fff3e0', borderRadius: '8px', padding: '8px 12px', marginBottom: '10px' }}>
                  <p style={{ fontSize: '12px', color: '#e65100', margin: 0 }}>
                    ⚠️ Advances exceed {r.salary_type === 'commission' ? 'commission' : 'salary'} by Rs. {Math.abs(r.remaining).toLocaleString()} — will carry forward.
                  </p>
                </div>
              )}
              ...
              {payingRider?.id === r.id && (
                <div style={{ marginTop: '14px', padding: '14px', background: '#f0f7ff', borderRadius: '10px', border: '1px solid #c8e0ff' }}>
                  <p style={{ fontSize: '13px', fontWeight: '700', color: '#0f4c81', marginBottom: '10px' }}>
      ...
      {activeTab === 'pending' && (
        <div>
          <p style={{ fontSize: '13px', color: '#888', marginBottom: '16px' }}>
            Advance requests sent directly to CEO/Admin.
          </p>
