...
export default function AdminDashboard({ user, onLogout }) {
    ...
    return (
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
          <p style={{ fontSize: '12px', color: '#888', fontWeight: '600', margin: 0, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{title}</p>
          <span style={{ fontSize: '22px' }}>{icon}</span>
        </div>
        <p style={{ fontSize: '28px', fontWeight: '700', color: '#1a1a2e', margin: '0 0 6px' }}>
          {prefix && <span style={{ fontSize: '16px', color: '#888', fontWeight: '400' }}>{prefix} </span>}
          {typeof value === 'number' ? value.toLocaleString() : value}
        </p>
        {subtitle && <p style={{ fontSize: '11px', color: '#aaa', margin: '0 0 8px' }}>{subtitle}</p>}
        {previous !== undefined && (
    ...
    return (
      <svg width="100%" viewBox={`0 0 ${totalW + 40} ${chartH + 50}`} style={{ overflow: 'visible' }}>
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => (
          <g key={i}>
            <line x1="30" y1={chartH - pct * chartH} x2={totalW + 40} y2={chartH - pct * chartH}
          ...
          return (
            <g key={i}>
              ...
              <rect x={x} y={chartH - barH} width={barW} height={barH} rx="4"
                fill={isToday ? '#0f4c81' : '#93c5fd'} />
              {/* Value label */}
              {d.value > 0 && (
                <text x={x + barW / 2} y={chartH - barH - 5} textAnchor="middle" fontSize="9" fill={isToday ? '#0f4c81' : '#888'} fontWeight={isToday ? '700' : '400'}>
                  {d.value >= 1000 ? (d.value / 1000).toFixed(1) + 'k' : d.value}
                </text>
              )}
              {/* Day label */}
              <text x={x + barW / 2} y={chartH + 16} textAnchor="middle" fontSize="9"
                fill={isToday ? '#0f4c81' : '#888'} fontWeight={isToday ? '700' : '400'}>
                {d.label.split(' ')[0]}
              </text>
              <text x={x + barW / 2} y={chartH + 28} textAnchor="middle" fontSize="9"
                fill={isToday ? '#0f4c81' : '#aaa'}>
                {d.label.split(' ')[1]}
              </text>
            </g>
          )
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
        <svg width="140" height="140">
          ...
          <text x={cx} y={cy + 10} textAnchor="middle" fontSize="13" fontWeight="700" fill="#333">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {[
            ...
            <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div>
                <p style={{ fontSize: '12px', color: '#555', margin: 0 }}>{item.label} ({item.pct}%)</p>
                <p style={{ fontSize: '13px', fontWeight: '700', color: '#333', margin: 0 }}>Rs. {item.value.toLocaleString()}</p>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif", background: '#f0f2f5' }}>
      ...
      {/* Sidebar */}
      <div style={{ width: '220px', background: '#0f4c81', color: 'white', display: 'flex', flexDirection: 'column', flexShrink: 0, boxShadow: '4px 0 20px rgba(0,0,0,0.15)' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        ...
        {/* Top Bar */}
        <div style={{ background: 'white', padding: '14px 28px', borderBottom: '1px solid #e8eaed', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
          <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            {alerts.pendingCashTransfers > 0 && (
              <button onClick={() => setActivePage('cashtransfer')}
                style={{ padding: '5px 12px', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: '20px', cursor: 'pointer', fontSize: '11px', color: '#ea580c', fontWeight: '600' }}>
                💸 {alerts.pendingCashTransfers} transfers
              </button>
            )}
            {alerts.pendingJazz > 0 && (
              <button onClick={() => setActivePage('jazzcash')}
                style={{ padding: '5px 12px', background: '#fdf4ff', border: '1px solid #e9d5ff', borderRadius: '20px', cursor: 'pointer', fontSize: '11px', color: '#9333ea', fontWeight: '600' }}>
                📱 Rs. {alerts.pendingJazz.toLocaleString()} pending
              </button>
            )}
            {alerts.pendingSalaryRequests > 0 && (
              <button onClick={() => setActivePage('salary')}
                style={{ padding: '5px 12px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '20px', cursor: 'pointer', fontSize: '11px', color: '#16a34a', fontWeight: '600' }}>
                💼 {alerts.pendingSalaryRequests} advances
              </button>
            )}
          </div>
        </div>
          ...
          {activePage === 'dashboard' && (
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div>
                  <h2 style={{ fontSize: '20px', fontWeight: '700', color: '#1a1a2e', margin: '0 0 4px' }}>Executive Summary</h2>
                  <p style={{ fontSize: '13px', color: '#888', margin: 0 }}>Comparing {dates?.label} vs {dates?.prevLabel}</p>
                </div>
                <div style={{ display: 'flex', gap: '6px', background: 'white', padding: '6px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                  {[
                    ...
                    <button key={p.key} onClick={() => setPeriod(p.key)}
                      style={{
                        ...
                        fontWeight: period === p.key ? '700' : '400', fontSize: '13px',
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '16px' }}>
                    <div style={{
                      ...
                      <div>
                        <p style={{ fontSize: '12px', opacity: 0.7, margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                          Total Revenue — {dates?.label}
                        </p>
                        <p style={{ fontSize: '42px', fontWeight: '700', margin: '0 0 8px', letterSpacing: '-1px' }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <p style={{ fontSize: '12px', opacity: 0.6, margin: '8px 0 0' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '16px' }}>
                    ...
                    <MetricCard title="JazzCash Sales" value={stats?.jazzSales} prefix="Rs." icon="📱"
                    ...
                    <MetricCard title="Total Receivable" value={alerts?.totalReceivable} prefix="Rs." icon="💰"
                      current={alerts?.totalReceivable} previous={alerts?.totalReceivable} color="#f59e0b"
                      onClick={() => setActivePage('reports')} subtitle="Click to view ledger" />
                  </div>

                  {/* ROW 3 — Operations Cards */}
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '20px' }}>
                    <MetricCard title="Pending Orders" value={alerts?.pendingOrders} icon="⏳"
                      color="#f59e0b" onClick={() => setActivePage('orders')} subtitle="Tap to assign riders" />
                    <MetricCard title="19L Bottles Sold" value={stats?.bottles19l} icon="🍶"
                    ...
                    <MetricCard title="Active Customers" value={alerts?.totalCustomers} icon="👥"
                      color="#1a7a4a" onClick={() => setActivePage('customers')} />
                    <MetricCard title="Cash Transfers" value={alerts?.pendingCashTransfers} icon="💸"
                      color="#ea580c" onClick={() => setActivePage('cashtransfer')}
                      subtitle={alerts?.pendingCashTransfers > 0 ? 'Pending confirmation' : 'All clear'} />
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px', marginBottom: '20px' }}>
                    ...
                    <div style={{ background: 'white', borderRadius: '14px', padding: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                        <div>
                          <h3 style={{ fontSize: '14px', fontWeight: '700', color: '#1a1a2e', margin: '0 0 2px' }}>Daily Sales — Last 7 Days</h3>
                          <p style={{ fontSize: '11px', color: '#888', margin: 0 }}>Dark bar = today</p>
                        </div>
                        <span style={{ fontSize: '11px', color: '#888', background: '#f0f2f5', padding: '4px 10px', borderRadius: '20px' }}>
                          Rs. {chartData.reduce((s, d) => s + d.value, 0).toLocaleString()} total
                        </span>
                      </div>
                    ...
                    {/* Donut Chart */}
                    <div style={{ background: 'white', borderRadius: '14px', padding: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
                      <h3 style={{ fontSize: '14px', fontWeight: '700', color: '#1a1a2e', margin: '0 0 4px' }}>Payment Methods</h3>
                      <p style={{ fontSize: '11px', color: '#888', margin: '0 0 16px' }}>{dates?.label}</p>
                      <DonutChart
                  ...
                  {(alerts.pendingOrders > 0 || alerts.pendingCashTransfers > 0 || alerts.pendingJazz > 0 || alerts.pendingSalaryRequests > 0) && (
                    <div style={{ background: 'white', borderRadius: '14px', padding: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
                      <h3 style={{ fontSize: '14px', fontWeight: '700', color: '#1a1a2e', margin: '0 0 14px' }}>⚠️ Action Required</h3>
                      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                        {alerts.pendingOrders > 0 && (
                          <div onClick={() => setActivePage('orders')}
                            style={{ flex: 1, minWidth: '180px', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: '10px', padding: '14px', cursor: 'pointer' }}>
                            <p style={{ fontSize: '20px', margin: '0 0 6px' }}>📦</p>
                            <p style={{ fontSize: '15px', fontWeight: '700', color: '#ea580c', margin: '0 0 2px' }}>{alerts.pendingOrders} Orders</p>
                            <p style={{ fontSize: '11px', color: '#888', margin: 0 }}>Waiting to be assigned to riders</p>
                          </div>
                        )}
                        {alerts.pendingCashTransfers > 0 && (
                          <div onClick={() => setActivePage('cashtransfer')}
                            style={{ flex: 1, minWidth: '180px', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: '10px', padding: '14px', cursor: 'pointer' }}>
                            <p style={{ fontSize: '20px', margin: '0 0 6px' }}>💸</p>
                            <p style={{ fontSize: '15px', fontWeight: '700', color: '#ea580c', margin: '0 0 2px' }}>{alerts.pendingCashTransfers} Transfers</p>
                            <p style={{ fontSize: '11px', color: '#888', margin: 0 }}>Riders waiting for cash confirmation</p>
                          </div>
                        )}
                        {alerts.pendingJazz > 0 && (
                          <div onClick={() => setActivePage('jazzcash')}
                            style={{ flex: 1, minWidth: '180px', background: '#fdf4ff', border: '1px solid #e9d5ff', borderRadius: '10px', padding: '14px', cursor: 'pointer' }}>
                            <p style={{ fontSize: '20px', margin: '0 0 6px' }}>📱</p>
                            <p style={{ fontSize: '15px', fontWeight: '700', color: '#9333ea', margin: '0 0 2px' }}>Rs. {alerts.pendingJazz.toLocaleString()}</p>
                            <p style={{ fontSize: '11px', color: '#888', margin: 0 }}>JazzCash awaiting confirmation</p>
                          </div>
                        )}
                        {alerts.pendingSalaryRequests > 0 && (
                          <div onClick={() => setActivePage('salary')}
                            style={{ flex: 1, minWidth: '180px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '10px', padding: '14px', cursor: 'pointer' }}>
                            <p style={{ fontSize: '20px', margin: '0 0 6px' }}>💼</p>
                            <p style={{ fontSize: '15px', fontWeight: '700', color: '#16a34a', margin: '0 0 2px' }}>{alerts.pendingSalaryRequests} Requests</p>
                            <p style={{ fontSize: '11px', color: '#888', margin: 0 }}>Salary advance requests pending</p>
                          </div>
                        )}
                      </div>
                    </div>
