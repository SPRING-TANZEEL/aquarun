...
export default function RiderCashTransfer({ rider }) {
  ...
  const [transferTo, setTransferTo] = useState(null) // 'main_rider' or 'office'
  ...
  async function fetchData() {
    ...
    const { data: mainRiderData } = await supabase
      .from('riders')
      .select('*')
      .eq('is_main_rider', true)
      .eq('is_active', true)
      .single()
    setMainRider(mainRiderData || null)

    // Get today's cash balance for this rider
    ...
    // Cash received from other riders (as main rider)
    ...
    // Cash already transferred out by this rider
    ...
    // Pending transfers — incoming (for main rider to confirm)
    ...
    // Also check pending transfers TO office (for admin)
    setLoading(false)
  }
  ...
  async function submitTransfer() {
    ...
    if (Number(amount) > cashBalance) return alert('Amount cannot be more than your cash balance: Rs. ' + cashBalance.toLocaleString())
    ...
    setSuccess({
      amount: Number(amount),
      to: isOffice ? 'Office' : mainRider?.full_name
    })
    setAmount('')
    setTransferTo(null)
    fetchData()
  ...
  return (
    <div>
      ...
      {success && (
        <div style={{ background: '#e8f5e9', border: '2px solid #4caf50', borderRadius: '10px', padding: '14px 16px', marginBottom: '16px' }}>
          ...
          <p style={{ fontSize: '13px', color: '#2e7d32', margin: 0 }}>
            Rs. {success.amount.toLocaleString()} sent to {success.to} — waiting for confirmation.
          </p>
      ...
      <div style={{
        ...
        padding: '18px', marginBottom: '12px', textAlign: 'center'
      }}>
        ...
        <div style={{ background: 'white', borderRadius: '12px', padding: '16px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: '2px solid #ffe082' }}>
          ...
          {pendingTransfers.map(t => (
            <div key={t.id} style={{
              ...
              padding: '12px', background: '#fffdf0', borderRadius: '8px',
              border: '1px solid #ffe082', marginBottom: '8px'
            }}>
              <div>
                <p style={{ fontSize: '14px', fontWeight: '600', margin: '0 0 2px' }}>
                  🚴 {t.from_rider?.full_name}
                </p>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button onClick={() => confirmTransfer(t)} disabled={confirming === t.id}
                    style={{
                      padding: '6px 12px', background: '#1a7a4a', color: 'white',
                      border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: '600'
                    }}>
      ...
      {cashBalance > 0 && (
        <div style={{ background: 'white', borderRadius: '12px', padding: '16px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <p style={{ fontSize: '13px', fontWeight: '700', color: '#555', marginBottom: '12px' }}>Return Cash To</p>

          <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
            {/* Main Rider option — only show if not main rider and main rider exists */}
            {!isMainRider && mainRider && (
              <button onClick={() => setTransferTo('main_rider')}
                style={{
                  flex: 1, padding: '16px', border: '2px solid',
                  borderColor: transferTo === 'main_rider' ? '#f59e0b' : '#eee',
                  borderRadius: '10px', cursor: 'pointer',
                  background: transferTo === 'main_rider' ? '#fff8e1' : '#f8f9fa',
            ...
            {/* Office option — always show, or only option if no main rider */}
            {(isMainRider || !mainRider) && (
              <button onClick={() => setTransferTo('office')}
                style={{
                  flex: 1, padding: '16px', border: '2px solid',
                  borderColor: transferTo === 'office' ? '#0f4c81' : '#eee',
                  borderRadius: '10px', cursor: 'pointer',
                  background: transferTo === 'office' ? '#e3f0ff' : '#f8f9fa',
                  display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px'
                }}>
                <span style={{ fontSize: '28px' }}>🏢</span>
            ...
            {/* Show both options for non-main riders when main rider exists */}
            {!isMainRider && mainRider && (
              <button onClick={() => setTransferTo('office')}
                style={{
                  flex: 1, padding: '16px', border: '2px solid',
                ...
                <span style={{ fontSize: '28px' }}>🏢</span>
                <span style={{ fontSize: '13px', fontWeight: '700', color: '#0f4c81' }}>Office</span>
                <span style={{ fontSize: '11px', color: '#888' }}>Admin/CEO</span>
              </button>
            )}
          </div>
          ...
          <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
            ...
            style={{
              ...
              boxSizing: 'border-box', textAlign: 'center', marginBottom: '8px'
            }} />
          <button onClick={() => setAmount(String(cashBalance))}
            style={{
              padding: '6px 14px', background: '#f0f4ff', border: '1px solid #d0d9ff',
              borderRadius: '6px', cursor: 'pointer', fontSize: '12px', color: '#0f4c81',
